...
func NewCLFFTPlan(ctx C.cl_context, dim int, dLengths []int) (*ClFFTPlan, error) {
	if dim < 1 {
	...
	if err := C.clfftCreateDefaultPlan(&outPlanHandle, ctx, C.clfftDim(dim), &cLengths[0]); err != C.CLFFT_SUCCESS {
...
//////////////// Abstract Functions ////////////////
func (FFTplan *ClFFTPlan) CopyPlan(ctx C.cl_context) (*ClFFTPlan, error) {
	var outPlanHandle C.clfftPlanHandle
	if err := C.clfftCopyPlan(&outPlanHandle, ctx, FFTplan.clFFTHandle); err != C.CLFFT_SUCCESS {
...
func (FFTplan *ClFFTPlan) BakePlanSimple(CommQueues []C.cl_command_queue) error {
	return toError(C.clfftBakePlan(FFTplan.clFFTHandle, C.cl_uint(len(CommQueues)), &CommQueues[0], nil, nil))
}

func (FFTplan *ClFFTPlan) BakePlanUnsafe(CommQueues []C.cl_command_queue, user_data unsafe.Pointer) error {
        return toError(C.CLFFTBakePlan(FFTplan.clFFTHandle, C.cl_uint(len(CommQueues)), &CommQueues[0], user_data))
}
...
func (FFTplan *ClFFTPlan) GetContext() (C.cl_context, error) {
	var outVal C.cl_context
	err := C.clfftGetPlanContext(FFTplan.clFFTHandle, &outVal)
	return outVal, toError(err)
}
...
func (FFTplan *ClFFTPlan) EnqueueForwardTransform(commQueues []C.cl_command_queue, WaitEventsList[]C.cl_event, input_buffers []C.cl_mem, output_buffers []C.cl_mem, tmpBuffer *C.cl_mem) ([]C.cl_event, error) {
	return FFTplan.EnqueueTransformUnsafe(commQueues, WaitEventsList, input_buffers, output_buffers, tmpBuffer, ClFFTDirectionForward)
}

func (FFTplan *ClFFTPlan) EnqueueBackwardTransform(commQueues []C.cl_command_queue, WaitEventsList[]C.cl_event, input_buffers []C.cl_mem, output_buffers []C.cl_mem, tmpBuffer *C.cl_mem) ([]C.cl_event, error) {
        return FFTplan.EnqueueTransformUnsafe(commQueues, WaitEventsList, input_buffers, output_buffers, tmpBuffer, ClFFTDirectionBackward)
}

func (FFTplan *ClFFTPlan) EnqueueTransformUnsafe(commQueues []C.cl_command_queue, WaitEventsList[]C.cl_event, input_buffers []C.cl_mem, output_buffers []C.cl_mem, tmpBuffer *C.cl_mem, TransformDirection ClFFTDirection) ([]C.cl_event, error) {
	queueLength := len(commQueues)
	outEvent := make([]C.cl_event, queueLength)
	var InDirection C.clfftDirection
	switch TransformDirection {
	default:
		InDirection = C.CLFFT_FORWARD
	case ClFFTDirectionForward:
		InDirection = C.CLFFT_FORWARD
	case ClFFTDirectionBackward:
		InDirection = C.CLFFT_BACKWARD
	}
	err := toError(C.clfftEnqueueTransform(FFTplan.clFFTHandle, InDirection, C.cl_uint(queueLength), &commQueues[0],
						C.cl_uint(len(WaitEventsList)), &WaitEventsList[0], &outEvent[0],
